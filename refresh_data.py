...
def _normalize_runners(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[df["馬名"].astype(str).str.strip() != ""].reset_index(drop=True)
    return df
...
def build_date_runners(
    ...
    for i, rid in enumerate(race_ids, 1):
        ...
        print(f"  [{i}/{len(race_ids)}] {rid} 出走{len(entries)}頭 結果{len(results)}")
        for e in entries:
            ...
            for k in ("着順1", "人気1", "着順2", "人気2", "着順3", "人気3",
                      "着順4", "人気4", "着順5", "人気5"):
                row[k] = score.get(k, "")
            rows.append(row)
...
def write_meta(dates: list[str], updated: list[str]) -> None:
    payload = {
...
def generate_predictions(dates: list[str] | None = None) -> None:
    if dates:
        ...
        subprocess.run(
            [sys.executable, "replay_predict.py", "--all"],
            check=True,
            timeout=1800,
        )


def latest_weekend(found: list[str]) -> list[str]:
    """最新開催日から同週末（差が2日以内）の開催日を返す。"""
    if not found:
        return []
    target = [found[0]]
    for d in found[1:]:
        delta = abs(
            (datetime.fromisoformat(found[0]) - datetime.fromisoformat(d)).days
        )
        if delta <= 2:
            target.append(d)
        else:
...
def refresh(
    ...
    if dates:
        ...
        if latest_only:
            target_dates = latest_weekend(found)
        else:
    ...
    save_runners(runners)
    av = available_dates(runners)
    write_meta(av, target_dates)
    if not skip_predict:
        missing = [
            d for d in av if not (PRED_DIR / f"predictions_{d}.csv").exists()
        ]
...
def main():
    ap = argparse.ArgumentParser(
        description="ARERU P0-2: 最新開催日取得 & runners/predictions 更新"
    )
